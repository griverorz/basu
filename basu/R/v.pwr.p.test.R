#' Vectorizes power calculations for proportion tests (one sample).
#'
#' \code {v.pwr.p.test} computes power of test or determine paramaters to obtain
...
#'   'method' elements. The argument(s) are assigned/computed
...
v.pwr.p.test <- function(h = NULL,
    ...
    out <- lapply(split(newargs, seq(nrow(newargs))),
                                function(x) do.call(pwr::pwr.p.test,x))
    if (deff != 1 & is.null(n)) {
